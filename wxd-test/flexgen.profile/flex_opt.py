...
class SelfAttention:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if i == 0:  # prefill
            mask, donate[1] = attention_mask.val.smart_copy(self.compute)
            h, new_k_cache, new_v_cache,attn_weights = self.compute.mha(h, mask, w_q, b_q,
            ...
            h, new_k_cache, new_v_cache,attn_weights = self.compute.mha_gen(h, mask, w_q,
                b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                k_cache, v_cache, donate, self.policy.attn_sparsity,
                self.policy.compress_cache, self.policy.comp_cache_config)
            cache_write_buf.store((new_k_cache, new_v_cache))
            if f"{self.layer_id}-layer" not in layer_attention_score:
                layer_attention_score[f"{self.layer_id}-layer"] =  {i:attn_weights.cpu()}
            else:
                layer_attention_score[f"{self.layer_id}-layer"][i] = attn_weights.cpu()
        hidden.val = h
